...
export default {
    ...
    pasteWithJson() {
      editor = atom.workspace.getActiveTextEditor()
      textSelected = editor.getSelectedText()
      clipText = atom.clipboard.read().replace('\r\n','')
      arr = JSON.parse(clipText)
      arr2 = textSelected.split('\r\n')
      result = ''
      arr2.forEach((lineStr, index)=>{
        arr.data.forEach((arrkeys,index2)=>{
          if(index===index2){
            arrkeys.keys.forEach((arritem)=>{
              let reg = new RegExp('{{'+arritem.key+'}}','g')
              lineStr = lineStr.replace(reg,arritem.value)
            })
          }
        })
        result += lineStr+'\r\n'
