...
export default {
    ...
    activate(state) {
        ...
        this.subscriptions.add(atom.commands.add('atom-workspace', {
            ...
            'atom-ywh-utils:pasteBefore': () => this.pasteBefore(),
            'atom-ywh-utils:pasteWithJson': () => this.pasteWithJson(),
            'atom-ywh-utils:pasteWithArray': () => this.pasteWithArray(),
            'atom-ywh-utils:trimSpacesAndTabs': () => this.trimSpacesAndTabs(),
        }));
    ...
    trimSpacesAndTabs() {
      editor = atom.workspace.getActiveTextEditor()
      textSelected = editor.getSelectedText()
      arr = textSelected.split('\r\n')
      result = ''
      arr.forEach((arrLines,i)=>{
        result+=arrLines.trimLeft()===''?'':(arrLines.trimLeft()+'\r\n')
      })
      editor.insertText(result.trimRight('\r\n'))
    }
